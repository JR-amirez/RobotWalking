...
const versions = {
    ...
    screenOrientation: getVersion('@capacitor/screen-orientation'),
};
...
if (fs.existsSync(capBuildGradlePath)) {
    ...
    const needsPatch = /project\(['"]:capacitor-/.test(capGradle);
    if (needsPatch) {
        capGradle = capGradle
            .replace(
            ...
            .replace(
                /implementation\s+project\(['"]:capacitor-screen-orientation['"]\)/,
                `implementation "com.capacitorjs:screen-orientation:${versions.screenOrientation}"`
            )
            .replace(
